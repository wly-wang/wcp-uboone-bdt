...
for (int i = 0; i < pfeval.reco_Ntrack; i++) {
  ...
  auto clean_nan = [](float &x, float fallback) {
    if (!std::isfinite(x)) x = fallback;
  };

  clean_nan(cos_theta, -999.);
  clean_nan(numu_1_score, -999.);
  clean_nan(numu_cc_3_track_length, -999.);
  clean_nan(numu_cc_3_max_length_all, -999.);
  clean_nan(cosmict_2_dQ_dx_front, -999.);
  clean_nan(cosmict_2_dQ_dx_end, -999.);
  clean_nan(cosmict_2_angle_beam, -999.);
  clean_nan(cosmict_2_phi, -999.);
  clean_nan(numu_cc_3_max_length, -999.);
  clean_nan(numu_cc_3_max_muon_length, -999.);
  clean_nan(has_reco_michel, -999.);
  ...
  if (raw_numubar > max_raw) max_raw = raw_numubar;

  double e_others  = std::exp(raw_others  - max_raw);
  double e_numu    = std::exp(raw_numu    - max_raw);
  double e_numubar = std::exp(raw_numubar - max_raw);
  double norm = e_others + e_numu + e_numubar;

  double p_others  = e_others / norm;
